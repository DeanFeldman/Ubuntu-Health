...
describe('Queue GET endpoints', () => {
  ...
  test('GET /api/queue/:clinicId with valid id returns 200 or 500', async () => {
...
describe('Queue POST endpoints', () => {
  ...
  test('POST /api/queue/:clinicId/join returns duplicate conflict or success for repeated join attempt', async () => {
    const res = await request(app)
      .post(`/api/queue/${validClinicId}/join`)
      .send({ patient_id: validPatientId, confirmed: true })
